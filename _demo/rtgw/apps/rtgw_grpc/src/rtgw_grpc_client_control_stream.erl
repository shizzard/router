-module(rtgw_grpc_client_control_stream).
-behaviour(gen_statem).

-compile([nowarn_untyped_record]).

-include_lib("rtgw/include/registry_definitions.hrl").
-include_lib("rtgw_grpc/include/rtgw_grpc.hrl").
-include_lib("rtgw_grpc/include/rtgw_grpc_client.hrl").
...
init({}) ->
  rtgw_log:component(rtgw_grpc_client),
  quickrand:seed(),
...
handle_event(state_timeout, ?msg_declare_stateless_services(), ?fsm_state_on_declare_stateless_services(), S0) ->
  ...
  ok = rtgw_grpc_client:await_ready(whereis(rtgw_grpc_client), 500),
  {ok, StreamRef} = rtgw_grpc_client:grpc_request(
    whereis(rtgw_grpc_client), self(), <<"lg.service.router.RegistryService">>, <<"RegisterVirtualService">>, #{}
  ),
  Pdu = #'lg.service.router.RegisterVirtualServiceRq'{
    virtual_service = #'lg.core.grpc.VirtualService'{
      service = {stateless, #'lg.core.grpc.VirtualService.StatelessVirtualService'{
        package = <<"lg.service.demo.rtgw">>,
        name = <<"GatewayService">>,
        methods = [
          #'lg.core.grpc.VirtualService.Method'{name = <<"SpawnTunnel">>}
        ]
      }},
  ...
  ok = rtgw_grpc_client:grpc_data(whereis(rtgw_grpc_client), StreamRef, _IsFin = true, Data),
...
handle_event(state_timeout, ?msg_init_control_stream(), ?fsm_state_on_init_control_stream(), S0) ->
  ...
  ok = rtgw_grpc_client:await_ready(whereis(rtgw_grpc_client), 500),
  {ok, StreamRef} = rtgw_grpc_client:grpc_request(
    whereis(rtgw_grpc_client), self(), <<"lg.service.router.RegistryService">>, <<"ControlStream">>, #{}
  ),
  ...
  Data = registry_definitions:encode_msg(Pdu),
  ok = rtgw_grpc_client:grpc_data(whereis(rtgw_grpc_client), StreamRef, Data),
  {keep_state, S0#state{stream_ref = StreamRef}};
